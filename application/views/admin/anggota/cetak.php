...
<body>
	<div class="cetak">
		<h1>DATA PROFIL <?php echo $anggota->nama ?></h1>
		<table class="table">
			<tbody>
				<tr>
					<td rowspan="14" width="200" style="border-right: solid thin #EEE;">
						<?php if($anggota->gambar != "") { ?>
							<img src="<?php echo base_url('assets/upload/anggota/'.$anggota->gambar) ?>" alt="<?php echo $anggota->nama ?>" class="foto" width="190">
						<?php }else{ ?>
							<img src="<?php echo base_url('assets/admin/dist/img/boxed-bg.jpg') ?>" alt="<?php echo $anggota->nama ?>" class="foto" width="190">
						<?php } ?>
					</td>
					<td width="25%">Nama</td>
					<td width="1%">:</td>
				...
				<tr>
					...
					<td width="1%">:</td>
					<td><?php echo $anggota->tempat_lahir.', '.date('d-m-Y',strtotime($anggota->tanggal_lahir)) ?></td>
				</tr>
				<tr>
					<td>Email</td>
					<td>:</td>
		...
		<pagebreak>
		<h1>DATA KURSUS <?php echo strtoupper($anggota->nama.' DI '.$site->namaweb) ?></h1>	
		<pagebreak>
		<h1>DATA PEMBAYARAN <?php echo strtoupper($anggota->nama.' DI '.$site->namaweb) ?></h1>
		<pagebreak>
		<h1>DATA KEHADIRAN <?php echo strtoupper($anggota->nama.' DI '.$site->namaweb) ?></h1>
	</div>
</body>
</html>