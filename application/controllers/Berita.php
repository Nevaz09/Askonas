...
class Berita extends CI_Controller {
	...
	// Main page
	public function index($jenis_berita = null)	{
		...
		if($jenis_berita == 'Profil') {
			$config['total_rows'] 		= count(array($this->berita_model->totalProfil()));
		}
		if($jenis_berita == 'Layanan') {
			$config['total_rows'] 		= count(array($this->berita_model->totalLayanan()));
		}
		if($jenis_berita == 'Tips') {
			$config['total_rows'] 		= count(array($this->berita_model->totalTips()));
		}
		if($jenis_berita == 'Kegiatan') {
			$config['total_rows'] 		= count(array($this->berita_model->totalKegiatan()));
		}
		if($jenis_berita == 'Berita' || is_null($jenis_berita)) {
		...
		if($jenis_berita == 'Profil') {
			$berita 	= $this->berita_model->listing_profil($config['per_page'], $page);
			$populer	= $this->berita_model->populer('Profil');
		}
		if($jenis_berita == 'Layanan') {
			$berita 	= $this->berita_model->listing_layanan($config['per_page'], $page);
			$populer	= $this->berita_model->populer('Layanan');
		}
		if($jenis_berita == 'Tips') {
			$berita 	= $this->berita_model->listing_tips($config['per_page'], $page);
			$populer	= $this->berita_model->populer('Tips');
		}
		if($jenis_berita == 'Kegiatan') {
			$berita 	= $this->berita_model->listing_kegiatan($config['per_page'], $page);
			$populer	= $this->berita_model->populer('Kegiatan');
		}
		if($jenis_berita == 'Berita' || is_null($jenis_berita)) {
	...
	public function profil($slug_berita = null)	{
		if(is_null($slug_berita)) return $this->index('Profil');
		$site 		= $this->konfigurasi_model->listing();
		$berita 	= $this->berita_model->read($slug_berita);
	...
	// Profil berita detail
	public function layanan($slug_berita = null)	{
		if(is_null($slug_berita)) return $this->index('Layanan');
	...
	public function tips($slug_berita = null)	{
		if(is_null($slug_berita)) return $this->index('Tips');
		
		$site 		= $this->konfigurasi_model->listing();
		$berita 	= $this->berita_model->read($slug_berita);
		$tips 	= $this->nav_model->nav_tips();

		if(count(array($berita)) < 1) {
			redirect(base_url('oops'),'refresh');
		}

		$listing 	= $this->berita_model->listing_tips();
	...
	public function kegiatan($slug_berita = null)	{
		if(is_null($slug_berita)) return $this->index('Tips');
		
		$site 		= $this->konfigurasi_model->listing();
		$berita 	= $this->berita_model->read($slug_berita);
		$kegiatan 	= $this->nav_model->nav_kegiatan();

		if(count(array($berita)) < 1) {
			redirect(base_url('oops'),'refresh');
		}

		$listing 	= $this->berita_model->listing_kegiatan();
